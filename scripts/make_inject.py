#!/usr/bin/env python3
"""
Genereer static/inject.js : een self-contained widget die het complete
index/register (filters + zoeken + resultaten EN de kaart-detailweergave)
client-side rendert en in een bestaande pagina mount. Bedoeld om via Scroll
Sites 'Custom JavaScript' te laden met een kleine loader:

    <script>var s=document.createElement('script');
      s.src='https://las.codeberg.page/playground/inject.js';
      s.charset='utf-8';document.head.appendChild(s);</script>

Alle data staat in het script (geen runtime fetch -> geen CORS-probleem).
Kaartklik -> detail in dezelfde container via hash-routing (#las=<uid>), dus
de gebruiker blijft op de Scroll-pagina. Pure ASCII uitvoer.

Invoer : content/index/*/index.md (front matter) + data/vocab.yaml
Uitvoer: static/inject.js  (wordt door Hugo als /playground/inject.js geserveerd)
"""
...
# vocab-map alleen voor gebruikte codes (code -> {label,kleur}) per veld
...
.twiin-las .card-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:1rem;}
...
.twiin-las .badge{display:inline-block;padding:.15rem .55rem;border-radius:999px;font-size:.74rem;font-weight:600;line-height:1.5;white-space:nowrap;background:var(--c-grijs-bg);color:var(--c-grijs-fg);}
...
# De app-logica. __TOKENS__ worden hieronder vervangen.
JS = r"""
(function(){
  var DATA=__DATA__, VOCAB=__VOCAB__;
  // Doelcontainer op de Scroll-pagina (eerste match wint); inhoud wordt vervangen.
  var SELECTORS=['.article-body.fb-layout-body','.fb-layout-container'];
  var HASHKEY='las';
  ...
  var slugToUid={}; DATA.forEach(function(c){slugToUid['tw-las-'+c.uid.split('-').slice(2).join('-').toLowerCase()]=c.uid; slugToUid[c.uid.toLowerCase()]=c.uid;});
  var state={q:'',filters:{}};
  var mount;
  ...
  function linkFor(url){ // interne /index/<slug>/ of #-hash -> in-widget kaart; anders extern
    var m=/(?:^|\/)index\/([a-z0-9-]+)\/?$/i.exec(url||'');
    if(m&&slugToUid[m[1].toLowerCase()])return '#'+HASHKEY+'='+slugToUid[m[1].toLowerCase()];
    return url;
  }
  ...
  function renderList(){
    ...
    var sort=(mount.querySelector('#tl-sort')||{}).value;
    if(sort==='ingang_desc')results.sort(function(a,b){return (b.ingangsdatum||'').localeCompare(a.ingangsdatum||'');});
    else if(sort==='ingang_asc')results.sort(function(a,b){return (a.ingangsdatum||'').localeCompare(b.ingangsdatum||'');});
    var base=DATA.filter(matches);
    var facetHtml=['<button type="button" class="btn" id="tl-reset">Filters wissen</button>'];
    FACETS.forEach(function(fp){var f=fp[0];var counts={};base.forEach(function(c){asList(c[f]).forEach(function(v){counts[v]=(counts[v]||0)+1;});});
      ...
      if(!vals.length)return; facetHtml.push('<div class="facet"><h3>'+esc(fp[1])+'</h3>');
      vals.forEach(function(v){facetHtml.push('<label class="facet__option"><input type="checkbox" data-facet="'+esc(f)+'" value="'+esc(v)+'"'+(active.indexOf(v)!==-1?' checked':'')+'><span>'+esc(lbl(f,v))+'</span><span class="facet__count">'+(counts[v]||0)+'</span></label>');});
      facetHtml.push('</div>');
    });
    var cardsHtml=results.length?results.map(function(c){
    ...
    mount.innerHTML='<div class="register"><aside class="register__filters">'+facetHtml.join('')+'</aside>'
      +'<div class="register__main"><form class="search" onsubmit="return false">'
      +'<input type="search" id="tl-search" placeholder="Zoek op vrije tekst, UID, naam" value="'+esc(state.q)+'">'
      +'<label class="search__sort">Sorteer: <select id="tl-sort"><option value="relevance">Relevantie</option><option value="ingang_desc">Ingangsdatum (nieuw-oud)</option><option value="ingang_asc">Ingangsdatum (oud-nieuw)</option></select></label></form>'
      +'<p class="register__stats">'+results.length+(results.length===1?' resultaat':' resultaten')+(state.q?(' voor "'+esc(state.q)+'"'):'')+'</p>'
      +'<div class="card-grid">'+cardsHtml+'</div></div></div>';
    if(sort)mount.querySelector('#tl-sort').value=sort;
    var si=mount.querySelector('#tl-search'); if(state.q){si.focus();si.setSelectionRange(si.value.length,si.value.length);}
  }

  // ---- detailweergave ----
  function para(body){ // minimale markdown: alinea's, [tekst](url), **vet**
    if(!body)return '';
    return body.split(/\n{2,}/).map(function(p){
  ...
  function renderDetail(uid){
    var c=byUid[uid]; if(!c){renderList();return;}
    var h=[];
    h.push('<article class="kaart"><button type="button" class="kaart__back" data-back>&larr; Terug naar index</button>');
  ...
  function route(){
    var m=new RegExp('[#&]'+HASHKEY+'=([^&]+)').exec(location.hash||'');
    if(m){renderDetail(decodeURIComponent(m[1]));window.scrollTo(0,0);}
    else renderList();
  }
  ...
  function wire(){
    mount.addEventListener('click',function(e){
      var back=e.target.closest&&e.target.closest('[data-back]'); if(back){if(location.hash){history.pushState('','',location.pathname+location.search);}route();return;}
      if(e.target.id==='tl-reset'){state.filters={};renderList();return;}
      var card=e.target.closest&&e.target.closest('.card[data-uid]');
      if(card){location.hash=HASHKEY+'='+card.getAttribute('data-uid');}
    });
    mount.addEventListener('input',function(e){if(e.target.id==='tl-search'){var v=e.target.value;clearTimeout(mount._t);mount._t=setTimeout(function(){state.q=v;renderList();},150);}});
    mount.addEventListener('change',function(e){
      if(e.target.id==='tl-sort'){renderList();return;}
      var cb=e.target.closest&&e.target.closest('input[data-facet]'); if(!cb)return;
  ...
  function start(host){
    css();
    mount=document.createElement('div'); mount.className='twiin-las'; mount.id='twiin-las-app';
    host.replaceChildren(mount);
    wire(); route();
    return true;
  }
  function find(){for(var i=0;i<SELECTORS.length;i++){var el=document.querySelector(SELECTORS[i]);if(el)return el;}return null;}
  function boot(){var h=find();if(h){start(h);return true;}return false;}
  if(!boot()){var mo=new MutationObserver(function(){if(boot())mo.disconnect();});mo.observe(document.documentElement,{childList:true,subtree:true});setTimeout(function(){mo.disconnect();},10000);}
})();
"""
